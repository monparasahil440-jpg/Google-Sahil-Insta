...
import { X, Send, Image as ImageIcon, Phone, Video, MessageCircle } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="bg-dark-secondary border border-dark-border w-full max-w-3xl h-[550px] rounded-2xl flex overflow-hidden shadow-2xl">
        ...
        <div className="w-72 border-r border-dark-border flex flex-col">
          ...
          <div className="flex-1 p-4 text-xs text-neutral-400 text-center flex items-center justify-center">
        ...
        <div className="flex-1 flex flex-col bg-dark-primary">
          ...
          <div className="h-14 border-b border-dark-border px-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-sm font-semibold text-white">Direct Chat</span>
            </div>

            <div className="flex items-center gap-2">
              <button
                onClick={() => startCall('friend', 'https://api.dicebear.com/7.x/avataaars/svg?seed=friend', 'voice')}
              ...
              <button
                onClick={() => startCall('friend', 'https://api.dicebear.com/7.x/avataaars/svg?seed=friend', 'video')}
                className="p-2 text-neutral-300 hover:text-white hover:bg-white/10 rounded-lg"
                title="Video Call"
              >
          ...
          <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-3">
            {messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-2 text-neutral-400 text-center my-auto">
                <MessageCircle className="w-12 h-12 stroke-1" />
                <span className="text-sm font-semibold text-white">Your Messages</span>
                <span className="text-xs">Send private photos and messages to a friend.</span>
              </div>
            ) : (
              messages.map((m) => (
                <div
